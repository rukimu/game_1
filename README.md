...
| `/inventory` | 所持品・装備管理（装備/解除・消耗品使用） |
...
- **職業診断クイズ**: 6問の診断で適性アーキタイプ（戦士/魔導士/盗賊/神官/吟遊詩人）に振り分け、自動生成された生い立ち（bio）も付与
- **シーズンの中心の謎**: 7つの手がかりを噂・戦闘・ダンジョンで発見し、最後の手がかりを引き当てた者がサーバー全体に告知される（最初の解明者にボーナス報酬）
...
- 経験値 / レベルアップ / 自動ステ強化
- インベントリ / 装備（スロット制・装備解除・戦闘時に補正反映 / 消耗品で街中HP/MP回復）
- 転職システム
  - Lv10/30/50で候補生成
...
- **ダンジョン**: テンプレート生成関数 `generateDungeonName` は実装済み。実際の探索フローは未実装。
...
- **NPC会話の街自動 seed**: 関数はあるが街生成時の自動付与は手動。
...
1. DM画面と通知
2. AIプロバイダ実装（Anthropic / OpenAI）+ 生成失敗時のフォールバック検証
3. 攻城戦の cron + バトル化
4. 闘技場の自動マッチング & レート
5. 取引（Trade）UI（提案・確認・承認）
6. 戦闘中の途中参加・途中退出フロー
7. シーズン切替時の世界再生成 (`WorldState` の cron)
8. Redis adapter for Socket.io（マルチノード対応）
9. Postgres前提の本番設定 + Stripe 連携 + ロール管理 (RBAC)
