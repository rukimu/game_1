...
- **戦闘状態異常**: 毒・火傷・スタンが debuff スキル / クリティカル時にかかる。tick ダメージ + 行動阻止
- **鍛冶 (アフィックス加工)**: `/forge` で装備のアフィックスをリロール、または強制でティアを 1 段階上げる。素材消費 5 個 + ゴールド
- **クエスト多様化**: 倒す・装備品供出・別の街へ伝令・連勝の腕試しの 4 種から自動生成
- **アチーブメント・称号**: 15 種のアチーブメント (ボス初討伐、呪い解除、決闘 25 勝、伝説装備入手、謎の解明者など)。獲得した称号は HUD に表示できる
- **攻城戦 MVP**: 各城に常に 1 つのシージが走る。24 時間の登録期間 + 3 時間の戦闘期間。ギルドのメンバー Lv + 決闘勝利数で決まるスコアで勝者が城を支配。サーバー全体に告知
