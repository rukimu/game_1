...
- **手作り固有職（curated）**: 「眼鏡戦士」「猫好き魔導師」「影語り」「老師」「鍛冶娘」「星詠み」等、12 体の手作り職業。各 80-150 字の signature bio + 装飾文字列の signature outfit + 1 つの unique skill（観察眼 / 招き猫の歌 / 影分身 等）を持つ。キャラ作成のクイズ結果から「あなたに似た固有職」を提示、`★固有` バッジ + `《outfit》` 表示で見分けられる。Phase 2 で AI 量産 (`docs/team/CURATED_JOB_BULK.md`) により 100 体まで増量予定
...
- **C31 Phase 2 (並行)**: curated job を 12 → 100 体まで AI 量産
